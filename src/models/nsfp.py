...
class NSFP(nn.Module):
    def __init__(self, filter_size=128, act_fn='relu', layer_size=8, \
                 itr_num=5000, lr=8e-3, min_delta=0.00005, early_patience=30, voxel_size=None, grid_feature_size=None,
                 verbose=False, point_cloud_range = [-51.2, -51.2, -3, 51.2, 51.2, 3], loss_fn='default'):
        super().__init__()
        ...
        print(f"\n---LOG[model]: NSFP loss_fn is {self.loss_fn}, itr_num: {itr_num}, lr: {lr}.")
    ...
    def cal_loss(self, dict2loss, net_inv, pc0_to_pc1):
        ...
        loss = 0
        if self.loss_fn == 'seflowLoss':
            res_loss = seflowLoss(dict2loss)
            for _, loss_name in enumerate(res_loss):
                loss += 1.0 * res_loss[loss_name]
        else:
        ...
        for batch_id in range(batch_sizes):
            ...
            with torch.inference_mode(False):
                with torch.enable_grad():
                    ...
                    if self.loss_fn != 'default':
                        dict2loss['pc0_dufopp_labels'] = batch['pc0_dynamic'][batch_id][~batch['gm0'][batch_id]][rm0]
                        dict2loss['pc1_dufopp_labels'] = batch['pc1_dynamic'][batch_id][~batch['gm1'][batch_id]][rm1]
