...
def check_flow(
    data_dir: str ="/home/ajinkya/datasets/truckscenes/man-truckscenes/preprocess/mini",
    res_name: str = "ssf_200_trucksc_13040213", # "flow", "flow_est"
    start_id: int = 0,
...
def vis_multiple(
    data_dir: str ="/home/ajinkya/datasets/truckscenes/man-truckscenes/preprocess/mini",
    res_name: list = ["flow", "fastnsf", "icpflow", "dogflow"],
    start_id: int = 400, # 3825,
    point_size: float = 3.0,
    vis_interval: int = 1,
    tone: str = 'dark',
    mode: str = "mul",
    weather: str = "all_weather",
):
    if mode != "mul":
        return
    assert isinstance(res_name, list), "vis_multiple() needs a list as flow_mode"
    dataset = HDF5Data(data_dir, vis_name=res_name, flow_view=True, weather=weather)
    ...
    while data_id >= 0 and data_id < len(dataset):
        ...
        for mode in res_name:
            ...
            if mode in ['dufo_label', 'label']:
                ...
                pcd.points = o3d.utility.Vector3dVector(pc0[~gm0, :3])
                flow = data[mode] - pose_flow # ego motion compensation here.
                flow_color = flow_to_rgb(flow) / 255.0
                is_dynamic = np.linalg.norm(flow, axis=1) > 0.3
