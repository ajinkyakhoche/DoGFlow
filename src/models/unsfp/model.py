...
def my_chamfer_fn(
    ...
    if x_lengths.item() == 0 or y_lengths.item() == 0:
        return 0.0, 0.0
...
def my_chamfer_fn_return_more(
    pc0,
    pc1
):
    _validate_chamfer_reduction_inputs("mean", "mean")

    pc0, pc0_lengths = _handle_pointcloud_input(pc0, None)
    pc1, pc1_lengths = _handle_pointcloud_input(pc1, None)

    if pc0_lengths.item() == 0 or pc1_lengths.item() == 0:
        return None, None, None, None

    # knn_points need batch size 
    pc0_nn = knn_points(pc0, pc1, lengths1=pc0_lengths, lengths2=pc1_lengths, K=1)
    pc1_nn = knn_points(pc1, pc0, lengths1=pc1_lengths, lengths2=pc0_lengths, K=1)

    dist0 = pc0_nn.dists[..., 0].squeeze()
    dist1 = pc1_nn.dists[..., 0].squeeze()
    idx0 = pc0_nn.idx[..., 0].squeeze()
    idx1 = pc1_nn.idx[..., 0].squeeze()
